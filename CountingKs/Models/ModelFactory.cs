    ...
    public class ModelFactory
    {
        ...
        public DiaryModel Create(Diary d)
        {
            return new DiaryModel()
            {
                ...
                Entries = d.Entries.Select(m=>Create(m))
            };
        }

        public DiaryEntryModel Create(DiaryEntry result)
        {
            return new DiaryEntryModel()
            {
                Url = _urlHelper.Link("DiaryEntries",new {diaryid=result.Diary.CurrentDate.ToString("yyyy-MM-dd"),id=result.Id}),
                FoodDescription = result.FoodItem.Description,
                MeasureDescription=result.Measure.Description,
                MeasureUrl = _urlHelper.Link("Measures",new {foodid=result.FoodItem.Id,id=result.Measure.Id}),
        ...
        public DiarySummaryModel CrateSummary(Diary diary)
        {
